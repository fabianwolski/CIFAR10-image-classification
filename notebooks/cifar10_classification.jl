...
# ╔═╡ 3a133821-739c-4da5-a619-df389e156ee8
begin
    #keeping dimensions for conv layers
x_data = reshape(train_x, 32, 32, 3, :)  
train_indices = 1:49000
val_indices = 49001:50000
#using tuples
train = [(x_data[:,:,:,i:i+999], train_labels[:,i:i+999]) 
             for i in 1:1000:49000] |> gpu
validate = [(x_data[:,:,:,i:i+999], train_labels[:,i:i+999]) 
			for i in 49001:1000:50000] |> gpu
#validation set variables for accuracy
validate_data = (x_data[:,:,:,val_indices] |> gpu,
                train_labels[:,val_indices] |> gpu)
(train, validate, validate_data)
end
...
with_terminal() do
	#testing the model up to flattening step
x = rand(Float32, 32, 32, 3, 1)#one image
model = Chain(
	Conv((5,5), 3=>16, pad=SamePad(), relu),
	MaxPool((2,2)),
...
# ╔═╡ 1615f78b-11e8-4f5f-b943-fdbbdebc003d
#TRAINING BASE MODEL:
#Time - 250secs
#Accuracy % - 63

#TRAINING WITH ADDED CONV + POOL LAYER:
#Time - 250secs
#Accuracy % - 63
#TRAINING WITH ADDED DENSE LAYER
#TIME 201,290,291
#ACCURACY - 64.5,66.7,66.9
#ACCURACY - 63%, 65%,65%,
...
# ╔═╡ 4fdae4aa-fe93-49f1-b432-554f168dda9f
begin
train_history = let
    correct = Float32[]
    epochs = 100
    for epoch = 1:epochs
        for d in train
                ...
                Flux.update!(optimiser, Flux.params(model), gs)
            end
            #chunks- had memory issues
            batch_size = 1000
            n_batches = size(validate_data[1], 4) ÷ batch_size
            acc = 0.0f0
            for i in 1:n_batches
                index_start = (i-1) * batch_size + 1
                index_end = i * batch_size
                batch_x = validate_data[1][:,:,:,index_start:index_end]
                batch_y = validate_data[2][:,index_start:index_end]
                acc += accuracy(batch_x, batch_y) * batch_size
            end
            acc = acc / size(validate_data[1], 4)
            push!(correct, acc)
            println(round(acc, digits=3))
        end
        correct
    end
#accuracy plot graph
plot(train_history, 
    ...
    title="Accuracy", 
    xlabel="epoch", 
    ylabel="porportion correct",
    size=(800, 600),
    linewidth=2)
end

# ╔═╡ 207fa0ed-3800-4220-bce3-f3b9c8aa5b40
model_a = Chain(
    Conv((5,5), 3=>16, pad=SamePad(), relu),
    MaxPool((2,2)),
    Conv((5,5), 16=>16, pad=SamePad(), relu),
...
# ╔═╡ 6a238e3b-6141-4449-9908-bc5da70f12ef
model_b = Chain(
    Conv((5,5), 3=>16, pad=SamePad(), relu),
    MaxPool((2,2)),
    Conv((5,5), 16=>8, pad=SamePad(), relu),
...
# ╔═╡ a0ab0850-0086-453a-8d3e-ff0d664ce563
begin
	#test data
   test_x, test_y = CIFAR10(split=:test)[:]
   test_labels = onehotbatch(test_y, 0:9)
   test_x_reshaped = reshape(test_x, 32, 32, 3, :)
	#test batching
    test = [(test_x_reshaped[:,:,:,i:i+999], test_labels[:,i:i+999])
	for i in 1:1000:10000] |> gpu
		n_samples = 10
	#sample data
	rand_indices = rand(1:size(test_x_reshaped, 4), n_samples)
	sample_images = test_x_reshaped[:,:,:,rand_indices]
	sample_labels = test_labels[:,rand_indices]
	# using onecold to convert from onehot
	true_classes = onecold(sample_labels, 0:9) .+ 1 #add 1 for conversion
	sample_class_names = classes[true_classes]
	#predict for sample images
	sample_predictions = cpu(model(gpu(sample_images)))
	test_data = (
		test=test,
...
# ╔═╡ 513aef2e-8326-410f-8aee-3dad1cd1c0fd
begin
	function show_predictions()
	predictions = test_data.sample_predictions
	col_names = [Symbol(lowercase(classes[argmax(predictions[:,i])]))
	for i in 1:size(predictions, 2)]
		df = DataFrame(
		predictions,
		col_names,
		makeunique=true)
	insertcols!(df, 1, "Classes/Actual" => classes)
...
# ╔═╡ 6c6ee0ce-794e-4233-95b7-0b62ce202328
begin
 @bind index Slider(1:1:10, default=1)
	function show_image(index)
	img = test_data.sample_images[:,:,:,index]
	true_label = test_data.sample_class_names[index]
	current_pred = test_data.sample_predictions[:,index]
	pred_label = classes[argmax(current_pred)]
	#using tick if prediction is correct
	title = pred_label == true_label ? "$pred_label ✓" : "$pred_label"
	(image(img), title)
	end
	show_image(index)
end
...
#arrays on CPU
	...
	#get prediction and move to CPU for the actual counting
	preds = cpu(model(x_batch))
	y_batch = cpu(y_batch)
	#convert to class indices on CPU
	pred_classes = onecold(preds, 0:9)
	true_classes = onecold(y_batch, 0:9)
	#counting correct prediction for each class
...
manifest_format = "2.0"
project_hash = "9dd22e3067c95413e087174cef10d0c1b631cbbe"
